...
import com.waz.service.assets2.{AssetPreviewService, ContentForUpload, RawAsset}
...
class AssetPreviewServiceImpl(implicit context: Context, ec: ExecutionContext) extends AssetPreviewService {
  ...
  override def extractPreview(rawAsset: RawAsset[General],
                              contentStream: => Future[InputStream]): Future[ContentForUpload] = {
    rawAsset.details match {
      case _: Video => extractVideoPreview(rawAsset, contentStream)
      case _ => Future.failed(NotSupportedError(s"Preview extraction for $rawAsset not supported"))
    }
  }

  def extractVideoPreview(rawAsset: RawAsset[General],
                          contentStream: => Future[InputStream]): Future[ContentForUpload] = {
    contentStream.flatMap { is =>
      val source: Source = Right(new InputStreamMediaDataSource(is))
      createMetadataRetriever(source).acquire { retriever =>
        Option(retriever.getFrameAtTime(-1L, OPTION_CLOSEST_SYNC)) match {
          ...
          case Some(frame) =>
            ...
            val content = ContentForUpload.Bytes(Mime.Image.Jpg, s"preview_${rawAsset.name}", compressedFrame)
